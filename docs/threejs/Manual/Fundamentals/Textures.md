Title: en/textures
Source URL: https://threejs.org/manual/en/textures.html

This article is one in a series of articles about three.js. The first article was [about three.js fundamentals](fundamentals.html). The [previous article](setup.html) was about setting up for this article. If you haven't read that yet you might want to start there.
...
![](../examples/resources/images/wall.jpg)

We'll modify one of our first samples. All we need to do is create a [`TextureLoader`](/docs/#api/en/loaders/TextureLoader). Call its [`load`](/docs/#api/en/loaders/TextureLoader#load) method with the URL of an image and set the material's `map` property to the result instead of setting its `color`.
...
Note that we're using [`MeshBasicMaterial`](/docs/#api/en/materials/MeshBasicMaterial) so no need for any lights.

[click here to open in a separate window](/manual/examples/textured-cube.html)

## 6 Textures, a different one on each face of a cube

How about 6 textures, one on each face of a cube?

![](../examples/resources/images/flower-1.jpg) ![](../examples/resources/images/flower-2.jpg) ![](../examples/resources/images/flower-3.jpg)

![](../examples/resources/images/flower-4.jpg) ![](../examples/resources/images/flower-5.jpg) ![](../examples/resources/images/flower-6.jpg)

We just make 6 materials and pass them as an array when we create the [`Mesh`](/docs/#api/en/objects/Mesh)
...
[click here to open in a separate window](/manual/examples/textured-cube-6-textures.html)

It should be noted though that not all geometry types supports multiple materials. [`BoxGeometry`](/docs/#api/en/geometries/BoxGeometry) can use 6 materials one for each face. [`ConeGeometry`](/docs/#api/en/geometries/ConeGeometry) can use 2 materials, one for the bottom and one for the cone. [`CylinderGeometry`](/docs/#api/en/geometries/CylinderGeometry) can use 3 materials, bottom, top, and side. For other cases you will need to build or load custom geometry and/or modify texture coordinates.

It's far more common in other 3D engines and far more performant to use a [Texture Atlas](https://en.wikipedia.org/wiki/Texture_atlas) if you want to allow multiple images on a single geometry. A Texture atlas is where you put multiple images in a single texture and then use texture coordinates on the vertices of your geometry to select which parts of a texture are used on each triangle in your geometry.

What are texture coordinates? They are data added to each vertex of a piece of geometry that specify what part of the texture corresponds to that specific vertex. We'll go over them when we start [building custom geometry](custom-buffergeometry.html).

## Loading Textures

### The Easy Way

Most of the code on this site uses the easiest method of loading textures. We create a [`TextureLoader`](/docs/#api/en/loaders/TextureLoader) and then call its [`load`](/docs/#api/en/loaders/TextureLoader#load) method. This returns a [`Texture`](/docs/#api/en/textures/Texture) object.
...
To wait for a texture to load the `load` method of the texture loader takes a callback that will be called when the texture has finished loading. Going back to our top example we can wait for the texture to load before creating our [`Mesh`](/docs/#api/en/objects/Mesh) and adding it to scene like this
...
[click here to open in a separate window](/manual/examples/textured-cube-wait-for-texture.html)

### Waiting for multiple textures to load

To wait until all textures have loaded you can use a [`LoadingManager`](/docs/#api/en/loaders/managers/LoadingManager). Create one and pass it to the [`TextureLoader`](/docs/#api/en/loaders/TextureLoader) then set its [`onLoad`](/docs/#api/en/loaders/managers/LoadingManager#onLoad) property to a callback.
...
The [`LoadingManager`](/docs/#api/en/loaders/managers/LoadingManager) also has an [`onProgress`](/docs/#api/en/loaders/managers/LoadingManager#onProgress) property we can set to another callback to show a progress indicator.
...
[click here to open in a separate window](/manual/examples/textured-cube-wait-for-all-textures.html)
...
For example [imgur](https://imgur.com), [flickr](https://flickr.com), and [github](https://github.com) all send headers allowing you to use images hosted on their servers in three.js. Most other websites do not.
...
![](../resources/images/compressed-but-large-wood-texture.jpg)

That image is only 157k so it will download relatively quickly but [it is actually 3024 x 3761 pixels in size](resources/images/compressed-but-large-wood-texture.jpg). Following the equation above that's
...
![](../resources/images/mip-low-res-enlarged.png)
...
![](../resources/images/mipmap-low-res-enlarged.png)
...
For setting the filter when the texture is drawn smaller than its original size you set the [`texture.minFilter`](/docs/#api/en/textures/Texture#minFilter) property to one of 6 values.
...
By default textures in three.js do not repeat. To set whether or not a texture repeats there are 2 properties, [`wrapS`](/docs/#api/en/textures/Texture#wrapS) for horizontal wrapping and [`wrapT`](/docs/#api/en/textures/Texture#wrapT) for vertical wrapping.
...
The last thing to note about the example is that if you change `wrapS` or `wrapT` on the texture you must also set [`texture.needsUpdate`](/docs/#api/en/textures/Texture#needsUpdate) so three.js knows to apply those settings. The other settings are automatically applied.

[click here to open in a separate window](/manual/examples/textured-cube-adjust.html)
