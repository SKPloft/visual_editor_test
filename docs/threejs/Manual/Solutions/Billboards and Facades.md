Title: en/billboards
Source URL: https://threejs.org/manual/en/billboards.html

In [a previous article](canvas-textures.html) we used a [`CanvasTexture`](/docs/#api/en/textures/CanvasTexture) to make labels / badges on characters. Sometimes we'd like to make labels or other things that always face the camera. Three.js provides the [`Sprite`](/docs/#api/en/objects/Sprite) and [`SpriteMaterial`](/docs/#api/en/materials/SpriteMaterial) to make this happen.

Let's change the badge example from [the article on canvas textures](canvas-textures.html) to use [`Sprite`](/docs/#api/en/objects/Sprite) and [`SpriteMaterial`](/docs/#api/en/materials/SpriteMaterial)
...
[click here to open in a separate window](/manual/examples/billboard-labels-w-sprites.html)

One problem is from certain angles the labels now intersect the characters.

![](../resources/images/billboard-label-z-issue.png)
...
[click here to open in a separate window](/manual/examples/billboard-labels-w-sprites-adjust-height.html)
...
Then we'll make a function that makes a [`Mesh`](/docs/#api/en/objects/Mesh) each for the trunk and top of a tree and parents both to an [`Object3D`](/docs/#api/en/core/Object3D).
...
[click here to open in a separate window](/manual/examples/billboard-trees-no-billboards.html)
...
Let's write some code to render an object to a texture using a `RenderTarget`. We covered rendering to a `RenderTarget` in [the article on render targets](rendertargets.html).
...
We're computing a box that contains the tree the same way we did in [the article on loading a .obj file](load-obj.html) with a few minor changes.

We call `frameArea` again adapted [the article on loading a .obj file](load-obj.html). In this case we compute how far the camera needs to be away from the object given its field of view to contain the object. We then position the camera -z that distance from the center of the box that contains the object.

We multiply the size we want to fit by 1.1 (`fudge`) to make sure the tree fits completely in the render target. The issue here is the size we're using to calculate if the object fits in the camera's view is not taking into account that the very edges of the object will end up dipping outside area we calculated. We could compute how to make 100% of the box fit but that would waste space as well so instead we just _fudge_ it.
...
[click here to open in a separate window](/manual/examples/billboard-trees-static-billboards.html)
