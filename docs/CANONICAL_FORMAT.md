...
- **Coordinate System**: The format uses a Y-up, right-handed coordinate system by default. The Unity exporter converts positions to Unity's Y-up, left-handed system by negating the Z component. Rotation handedness conversion for non-trivial orientations remains unresolved technical debt; see `docs/TECHNICAL_DEBT.md`.
