Title: en/animation-system
Source URL: https://threejs.org/manual/en/animation-system.html
...
Note that not all model formats include animation (OBJ notably does not), and that only some three.js loaders support \`AnimationClip\` sequences. Several that _do_ support this animation type:
