Title: en/webgpurenderer
Source URL: https://threejs.org/manual/en/webgpurenderer.html
...
Most common methods known from \`WebGLRenderer\` like \`clear()\`, \`setRenderTarget()\`or \`dispose()\` are also present in \`WebGPURenderer\`. Please have a look at the [API documentation](https://threejs.org/docs/#Renderer) for a full overview of the renderer's public interface.
