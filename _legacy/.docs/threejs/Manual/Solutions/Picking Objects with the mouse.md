Title: en/picking
Source URL: https://threejs.org/manual/en/picking.html
...
Let's make a scene with a 100 objects and try picking them. We'll start with an example from [the article on responsive pages](responsive.html)
...
Of course we could call this function only when the user pressed the mouse _down_ which is probably usually what you want but for this example we'll pick every frame whatever is under the mouse. To do this we first need to track where the mouse is
...
[click here to open in a separate window](/manual/examples/picking-raycaster.html)
...
![](../examples/resources/images/frame.png)
...
[click here to open in a separate window](/manual/examples/picking-raycaster-transparency.html)

Try to pick something through a box and you can't

![](../resources/images/picking-transparent-issue.jpg)
...
Note that we are abusing the [`MeshPhongMaterial`](/docs/#api/en/materials/MeshPhongMaterial) here. By setting its `emissive` to our id and the `color` and `specular` to 0 that will end up rendering the id only where the texture's alpha is greater than `alphaTest`. We also need to set `blending` to `NoBlending` so that the id is not multiplied by alpha.

Note that abusing the [`MeshPhongMaterial`](/docs/#api/en/materials/MeshPhongMaterial) might not be the best solution as it will still calculate all our lights when drawing the picking scene even though we don't need those calculations. A more optimized solution would make a custom shader that just writes the id where the texture's alpha is greater than `alphaTest`.
...
Then let's change the `PickHelper` into a `GPUPickHelper`. It will use a [`WebGLRenderTarget`](/docs/#api/en/renderers/WebGLRenderTarget) like we covered the [article on render targets](rendertargets.html). Our render target here is only a single pixel in size, 1x1.
...
[click here to open in a separate window](/manual/examples/picking-gpu.html)
