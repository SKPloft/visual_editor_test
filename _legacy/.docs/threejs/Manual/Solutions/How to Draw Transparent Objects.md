...
Source URL: https://threejs.org/manual/en/transparency.html
...
We'll start with the example from [the article on rendering on demand](rendering-on-demand.html) which had 3 cubes and modify it to have 8. First let's change our `makeInstance` function to take an x, y, and z
...
To make the cubes transparent we just need to set the [`transparent`](/docs/#api/en/materials/Material#transparent) flag and to set an [`opacity`](/docs/#api/en/materials/Material#opacity) level with 1 being completely opaque and 0 being completely transparent.
...
[click here to open in a separate window](/manual/examples/transparency.html)

Drag on the example to rotate the view.

So it seems easy but ... look closer. The cubes are missing their backs.

![](../resources/images/transparency-cubes-no-backs.png)

no backs

We learned about the [`side`](/docs/#api/en/materials/Material#side) material property in [the article on materials](materials.html). So, let's set it to `THREE.DoubleSide` to get both sides of each cube to be drawn.
...
[click here to open in a separate window](/manual/examples/transparency-doubleside.html)

Give it a spin. It kind of looks like it's working as we can see backs except on closer inspection sometimes we can't.

![](../resources/images/transparency-cubes-some-backs.png)
...
The solution is to sort transparent things and draw the stuff in back before drawing the stuff in front. THREE.js does this for objects like [`Mesh`](/docs/#api/en/objects/Mesh) otherwise the very first example would have failed between cubes with some cubes blocking out others. Unfortunately for individual triangles shorting would be extremely slow.

The cube has 12 triangles, 2 for each face, and the order they are drawn is [the same order they are built in the geometry](custom-buffergeometry.html) so depending on which direction we are looking the triangles closer to the camera might get drawn first. In that case the triangles in the back aren't drawn. This is why sometimes we don't see the backs.
...
[click here to open in a separate window](/manual/examples/transparency-doubleside-hack.html)
...
Let's make 2 intersecting planes (after deleting all the code related to cubes). We'll [add a texture](textures.html) to each plane.
...
How you accomplish that is up to you. If I was using modeling package like [Blender](https://blender.org) I'd probably do this manually by adjusting texture coordinates. Here though we're using [`PlaneGeometry`](/docs/#api/en/geometries/PlaneGeometry) which by default stretches the texture across the plane. Like we [covered before](textures.html) By setting the [`texture.repeat`](/docs/#api/en/textures/Texture#repeat) and [`texture.offset`](/docs/#api/en/textures/Texture#offset) we can scale and move the texture to get the correct half of the face texture on each plane.

The code above also makes a [`Object3D`](/docs/#api/en/core/Object3D) and parents the 2 planes to it. It seemed easier to rotate a parent [`Object3D`](/docs/#api/en/core/Object3D) than to do the math required do it without.

[click here to open in a separate window](/manual/examples/transparency-intersecting-planes-fixed.html)
...
An alpha test is a level of _alpha_ below which three.js will not draw the pixel. If we don't draw a pixel at all then the depth issues mentioned above disappear. For relatively sharp edged textures this works pretty well. Examples include leaf textures on a plant or tree or often a patch of grass.

Let's try on the 2 planes. First let's use different textures. The textures above were 100% opaque. These 2 use transparency.

![](../examples/resources/images/tree-01.png)

![](../examples/resources/images/tree-02.png)

Going back to the 2 planes that intersect (before we split them) let's use these textures and set an [`alphaTest`](/docs/#api/en/materials/Material#alphaTest).
...
Before we run this let's add a small UI so we can more easily play with the `alphaTest` and `transparent` settings. We'll use lil-gui like we introduced in the [article on three.js's scenegraph](scenegraph.html).
...
[click here to open in a separate window](/manual/examples/transparency-intersecting-planes-alphatest.html)

You can see it works but zoom in and you'll see one plane has white lines.

![](../resources/images/transparency-alphatest-issues.png)
