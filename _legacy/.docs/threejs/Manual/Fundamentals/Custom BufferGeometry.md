Title: en/custom-buffergeometry
Source URL: https://threejs.org/manual/en/custom-buffergeometry.html

[`BufferGeometry`](/docs/#api/en/core/BufferGeometry) is three.js's way of representing all geometry. A [`BufferGeometry`](/docs/#api/en/core/BufferGeometry) essentially a collection _named_ of [`BufferAttribute`](/docs/#api/en/core/BufferAttribute)s. Each [`BufferAttribute`](/docs/#api/en/core/BufferAttribute) represents an array of one type of data: positions, normals, colors, uv, etc... Together, the named [`BufferAttribute`](/docs/#api/en/core/BufferAttribute)s represent _parallel arrays_ of all the data for each vertex.

![](../resources/threejs-attributes.svg)

Above you can see we have 4 attributes: `position`, `normal`, `color`, `uv`. They represent _parallel arrays_ which means that the Nth set of data in each attribute belongs to the same vertex. The vertex at index = 4 is highlighted to show that the parallel data across all attributes defines one vertex.

This brings up a point, here's a diagram of a cube with one corner highlighted.

![](../resources/cube-faces-vertex.svg)
...
A single _vertex_ is the combination of all of its parts. If a vertex needs any part to be different then it must be a different vertex.

As a simple example let's make a cube using [`BufferGeometry`](/docs/#api/en/core/BufferGeometry). A cube is interesting because it appears to share vertices at the corners but really does not. For our example we'll list out all the vertices with all their data and then convert that data into parallel arrays and finally use those to make [`BufferAttribute`](/docs/#api/en/core/BufferAttribute)s and add them to a [`BufferGeometry`](/docs/#api/en/core/BufferGeometry).
...
Finally we can create a [`BufferGeometry`](/docs/#api/en/core/BufferGeometry) and then a [`BufferAttribute`](/docs/#api/en/core/BufferAttribute) for each array and add it to the [`BufferGeometry`](/docs/#api/en/core/BufferGeometry).
...
Above we created 3 JavaScript native arrays, `positions`, `normals` and `uvs`. We then convert those into [TypedArrays](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray) of type `Float32Array`. A [`BufferAttribute`](/docs/#api/en/core/BufferAttribute) requires a TypedArray not a native array. A [`BufferAttribute`](/docs/#api/en/core/BufferAttribute) also requires you to tell it how many components there are per vertex. For the positions and normals we have 3 components per vertex, x, y, and z. For the UVs we have 2, u and v.

[click here to open in a separate window](/manual/examples/custom-buffergeometry-cube.html)
...
So now we have 24 unique vertices. Then we specify 36 indices for the 36 vertices we need drawn to make 12 triangles by calling [`BufferGeometry.setIndex`](/docs/#api/en/core/BufferGeometry.setIndex) with an array of indices.
...
[click here to open in a separate window](/manual/examples/custom-buffergeometry-cube-typedarrays.html)
...
Because I'm lazy I used a small hierarchy of 3 [`Object3D`](/docs/#api/en/core/Object3D) objects to compute sphere points. How this works is explained in [the article on optimizing lots of objects](optimize-lots-of-objects.html).
...
[click here to open in a separate window](/manual/examples/custom-buffergeometry-dynamic.html)

I hope these were useful examples of how to use [`BufferGeometry`](/docs/#api/en/core/BufferGeometry) directly to make your own geometry and how to dynamically update the contents of a [`BufferAttribute`](/docs/#api/en/core/BufferAttribute).
