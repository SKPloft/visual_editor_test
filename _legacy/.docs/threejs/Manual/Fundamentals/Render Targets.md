Title: en/rendertargets
Source URL: https://threejs.org/manual/en/rendertargets.html
...
Rendering to a render target is almost exactly the same as normal rendering. First we create a [`WebGLRenderTarget`](/docs/#api/en/renderers/WebGLRenderTarget).

const rtWidth = 512;
const rtHeight = 512;
const renderTarget = new THREE.WebGLRenderTarget(rtWidth, rtHeight);

Then we need a [`Camera`](/docs/#api/en/cameras/Camera) and a [`Scene`](/docs/#api/en/scenes/Scene)
...
The [`Scene`](/docs/#api/en/scenes/Scene) and [`Camera`](/docs/#api/en/cameras/Camera) from the previous article are still there. We'll use them to render to the canvas. We just need to add stuff to render.
...
Render targets are used for all kinds of things. [Shadows](shadows.html) use render targets. [Picking can use a render target](picking.html). Various kinds of [post processing effects](post-processing.html) require render targets. Rendering a rear view mirror in a car or a live view on a monitor inside a 3D scene might use a render target.

A few notes about using [`WebGLRenderTarget`](/docs/#api/en/renderers/WebGLRenderTarget).

*   By default [`WebGLRenderTarget`](/docs/#api/en/renderers/WebGLRenderTarget) creates 2 textures. A color texture and a depth/stencil texture. If you don't need the depth or stencil textures you can request to not create them by passing in options. Example:
