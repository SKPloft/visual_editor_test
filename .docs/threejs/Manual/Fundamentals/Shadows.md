Title: en/shadows
Source URL: https://threejs.org/manual/en/shadows.html

This article is part of a series of articles about three.js. The first article is [three.js fundamentals](fundamentals.html). If you haven't read that yet and you're new to three.js you might want to consider starting there. The [previous article was about cameras](cameras.html) which is important to have read before you read this article as well as the [article before that one about lights](lights.html).
...
Three.js by default uses _shadow maps_. The way a shadow map works is, _for every light that casts shadows all objects marked to cast shadows are rendered from the point of view of the light_. **READ THAT AGAIN!** and let it sink in.
...
![](../examples/resources/images/roundshadow.png)
...
Then we'll setup the same checkerboard ground but this time it's using a [`MeshBasicMaterial`](/docs/#api/en/materials/MeshBasicMaterial) as we don't need lighting for the ground.
...
Now we'll make a bunch of spheres. For each sphere we'll create a `base` [`THREE.Object3D`](/docs/#api/en/core/Object3D) and we'll make both the shadow plane mesh and the sphere mesh children of the base. That way if we move the base both the sphere and the shadow will move. We need to put the shadow slightly above the ground to prevent z-fighting. We also set `depthWrite` to false so that the shadows don't mess each other up. We'll go over both of these issues in [another article](transparency.html). The shadow is a [`MeshBasicMaterial`](/docs/#api/en/materials/MeshBasicMaterial) because it doesn't need lighting.
...
We setup 2 lights. One is a [`HemisphereLight`](/docs/#api/en/lights/HemisphereLight) with the intensity set to 2 to really brighten things up.
...
The other is a [`DirectionalLight`](/docs/#api/en/lights/DirectionalLight) so the spheres get some definition
...
[click here to open in a separate window](/manual/examples/shadows-fake.html)

In some apps it's common to use a round or oval shadow for everything but of course you could also use different shaped shadow textures. You might also give the shadow a harder edge. A good example of using this type of shadow is [Animal Crossing Pocket Camp](https://www.google.com/search?tbm=isch&q=animal+crossing+pocket+camp+screenshots) where you can see each character has a simple round shadow. It's effective and cheap. [Monument Valley](https://www.google.com/search?q=monument+valley+screenshots&tbm=isch) appears to also use this kind of shadow for the main character.

So, moving on to shadow maps, there are 3 lights which can cast shadows. The [`DirectionalLight`](/docs/#api/en/lights/DirectionalLight), the [`PointLight`](/docs/#api/en/lights/PointLight), and the [`SpotLight`](/docs/#api/en/lights/SpotLight).

Let's start with the [`DirectionalLight`](/docs/#api/en/lights/DirectionalLight) with the helper example from [the lights article](lights.html).
...
Let's make the plane (the ground) only receive shadows since we don't really care what happens underneath.
...
[click here to open in a separate window](/manual/examples/shadows-directional-light.html)

What happened? Why are parts of the shadows missing?

The reason is shadow maps are created by rendering the scene from the point of view of the light. In this case there is a camera at the [`DirectionalLight`](/docs/#api/en/lights/DirectionalLight) that is looking at its target. Just like [the camera's we previously covered](cameras.html) the light's shadow camera defines an area inside of which the shadows get rendered. In the example above that area is too small.

In order to visualize that area we can get the light's shadow camera and add a [`CameraHelper`](/docs/#api/en/helpers/CameraHelper) to the scene.
...
[click here to open in a separate window](/manual/examples/shadows-directional-light-with-camera-helper.html)
...
Let's add some GUI setting to adjust the light's shadow camera box. Since a [`DirectionalLight`](/docs/#api/en/lights/DirectionalLight) represents light all going in a parallel direction, the [`DirectionalLight`](/docs/#api/en/lights/DirectionalLight) uses an [`OrthographicCamera`](/docs/#api/en/cameras/OrthographicCamera) for its shadow camera. We went over how an [`OrthographicCamera`](/docs/#api/en/cameras/OrthographicCamera) works in [the previous article about cameras](cameras.html).

Recall an [`OrthographicCamera`](/docs/#api/en/cameras/OrthographicCamera) defines its box or _view frustum_ by its `left`, `right`, `top`, `bottom`, `near`, `far`, and `zoom` properties.
...
We'll also use the `MinMaxGUIHelper` we created in the [camera article](cameras.html) to adjust `near` and `far`.
...
[click here to open in a separate window](/manual/examples/shadows-directional-light-with-camera-gui.html)
...
![](../resources/images/low-res-shadow-map.png)
...
You can set the resolution of the shadow map's texture by setting `light.shadow.mapSize.width` and `light.shadow.mapSize.height`. They default to 512x512. The larger you make them the more memory they take and the slower they are to compute so you want to set them as small as you can and still make your scene work. The same is true with the light's shadow camera area. Smaller means better looking shadows so make the area as small as you can and still cover your scene. Be aware that each user's machine has a maximum texture size allowed which is available on the renderer as [`renderer.capabilities.maxTextureSize`](/docs/#api/en/renderers/WebGLRenderer#capabilities).

Switching to the [`SpotLight`](/docs/#api/en/lights/SpotLight) the light's shadow camera becomes a [`PerspectiveCamera`](/docs/#api/en/cameras/PerspectiveCamera). Unlike the [`DirectionalLight`](/docs/#api/en/lights/DirectionalLight)'s shadow camera where we could manually set most its settings, [`SpotLight`](/docs/#api/en/lights/SpotLight)'s shadow camera is controlled by the [`SpotLight`](/docs/#api/en/lights/SpotLight) itself. The `fov` for the shadow camera is directly connected to the [`SpotLight`](/docs/#api/en/lights/SpotLight)'s `angle` setting. The `aspect` is set automatically based on the size of the shadow map.

\-const light = new THREE.DirectionalLight(color, intensity);
+const light = new THREE.SpotLight(color, intensity);

and we added back in the `penumbra` and `angle` settings from our [article about lights](lights.html).

[click here to open in a separate window](/manual/examples/shadows-spot-light-with-camera-gui.html)

And finally there's shadows with a [`PointLight`](/docs/#api/en/lights/PointLight). Since a [`PointLight`](/docs/#api/en/lights/PointLight) shines in all directions the only relevant settings are `near` and `far`. Otherwise the [`PointLight`](/docs/#api/en/lights/PointLight) shadow is effectively 6 [`SpotLight`](/docs/#api/en/lights/SpotLight) shadows each one pointing to the face of a cube around the light. This means [`PointLight`](/docs/#api/en/lights/PointLight) shadows are much slower since the entire scene must be drawn 6 times, one for each direction.
...
And of course we need to switch the light to a [`PointLight`](/docs/#api/en/lights/PointLight).
...
[click here to open in a separate window](/manual/examples/shadows-point-light.html)
