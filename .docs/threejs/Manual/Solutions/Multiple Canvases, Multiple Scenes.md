...
The solution is one canvas that fills the viewport in the background and some other element to represent each "virtual" canvas. We make a single [`Renderer`](/docs/#api/en/constants/Renderer) and then one [`Scene`](/docs/#api/en/scenes/Scene) for each virtual canvas. We'll then check the positions of the virtual canvas elements and if they are on the screen we'll tell THREE.js to draw their scene at the correct place.
...
And then we'll make a function to render each scene only if the element is on the screen. We can tell THREE.js to only render to part of the canvas by turning on the _scissor_ test with [`Renderer.setScissorTest`](/docs/#api/en/constants/Renderer.setScissorTest) and then setting both the scissor and the viewport with [`Renderer.setViewport`](/docs/#api/en/constants/Renderer.setViewport) and [`Renderer.setScissor`](/docs/#api/en/constants/Renderer.setScissor).
...
[click here to open in a separate window](/manual/examples/multiple-scenes-v1.html)
...
And if we [quickly scroll up and down](../examples/multiple-scenes-v2.html) we'll see the issue. Here's an animation of scrolling slowed down by 10x.

![](../resources/images/multi-view-skew.gif)
...
![](../resources/images/multi-view-fixed.gif)
...
[click here to open in a separate window](/manual/examples/multiple-scenes-generic.html)
...
[click here to open in a separate window](/manual/examples/multiple-scenes-controls.html)
...
[click here to open in a separate window](/manual/examples/multiple-scenes-copy-canvas.html)

One other advantage to this solution is you could potentially use [`OffscreenCanvas`](https://developer.mozilla.org/en-US/docs/Web/API/OffscreenCanvas) to render from a web worker and still use this technique.